...
from onlinepainting_climbing_and_manipulator_planner import *
...
class RenovationRobot():
    ...
    def renovationrobot_motion_and_visualization(self,planning_source_dict,rate):
        ...
        while not rospy.is_shutdown():
            ...
            "plan the aubo robot arm paths and manipulator base coordinate frame"
            # the inputs are: 
            # tf data: the reached mobile base position 
            # the planned painting waypaths
            renovation_mobilebase_position_onecell=[]
            listener = tf.TransformListener()
            while not rospy.is_shutdown():
                try:
                    (trans,rot) = listener.lookupTransform("base_link", "map", rospy.Time(0))
                    angle = tf.transformations.euler_from_quaternion(rot)
                    renovation_mobilebase_position_onecell=[trans[0],trans[1],trans[2],angle[0],angle[1],angle[2]]
                except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                    continue
                if len(renovation_mobilebase_position_onecell)>0:
                    break

            if plane_num_count!=1 and plane_num_count!=3:    
                 renovation_rodclimbing_motiondistance, renovation_manipulatorwaypoint_jointslist = onlinepainting_climbing_and_manipulator_planner(renovation_mobilebase_position_onecell, plane_num_count, mobile_base_point_count)
            ...
            "execute motions of climbing mechanism and manipulator"
            while not rospy.is_shutdown():
                if plane_num_count!=1 and plane_num_count!=3:
                    ...
                    time1=time.time()
                    rodclimb_mechanism_motion(0.0,renovation_rodclimbing_motiondistance[climb_base_count_num],rate)
                    ...
                    aubo_q_list=renovation_manipulatorwaypoint_jointslist
