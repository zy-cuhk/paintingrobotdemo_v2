...
def obtain_waypaths_insideclimbingworkspace1(plane_num, cell_num, candidate_manipulatorbase_position,renovation_waypaths_onecell,paintinggun_T):
    rmax=1.7
    climbingjoints_coverage_number=np.zeros(len(candidate_manipulatorbase_position))
    cartersianwaypaths_incandidateclimbingjoints=defaultdict(defaultdict)
    cartersianwaypaths_outof_candidateclimbingjoints=defaultdict(defaultdict)
...
def onlinepainting_climbing_and_manipulator_planner(renovation_mobilebase_position_onecell, plane_num_count, mobile_base_point_count):
    ...
    renovation_rodclimbing_motiondistance=multidict()
    renovation_manipulatorwaypoint_jointslist=multidict()
    renovation_manipualtorwaypoint_cartesianlist=multidict()

    i=plane_num_count
    j=mobile_base_point_count
    if i==2 and j==0:
        print("renovation_mobilebase_position_onecell before is:",renovation_mobilebase_position_onecell)
        offset_length1=-0.04
        p1=renovation_mobilebase_position_onecell
        theta1=p1[5]
        renovation_mobilebase_position_onecell=[p1[0]+offset_length1*cos(theta1+pi/2),p1[1]+offset_length1*sin(theta1+pi/2),p1[2],p1[3],p1[4],p1[5]]
        print("renovation_mobilebase_position_onecell after is:",renovation_mobilebase_position_onecell)
    if i==4 and j==0:
        print("renovation_mobilebase_position_onecell before is:",renovation_mobilebase_position_onecell)
        offset_length3=-0.04
        p5=renovation_mobilebase_position_onecell
        theta3=p5[5]
        renovation_mobilebase_position_onecell=[p5[0]+offset_length3*cos(theta3+pi/2),p5[1]+offset_length3*sin(theta3+pi/2),p5[2],p5[3],p5[4],p5[5]]
        print("renovation_mobilebase_position_onecell after is:",renovation_mobilebase_position_onecell)
    ...
    # for k in range(3):
        ...
        climbingjoints_coverage_number, cartersianwaypaths_incandidateclimbingjoints, cartersianwaypaths_outof_candidateclimbingjoints = obtain_waypaths_insideclimbingworkspace1(i,j, candidate_manipulatorbase_position,renovation_waypaths_onecell,paintinggun_T)
        ...
        renovation_manipulatorbase_positions[manipulatorbase_num_inonecell]=selected_manipulatorbase_position[0:6]

        renovation_rodclimbing_motiondistance[manipulatorbase_num_inonecell]=selected_manipulatorbase_position[2]-1.48

        for coverage_waypoints_num in range(len(scheduled_selectedjoints_dict)):
            renovation_manipulatorwaypoint_jointslist[manipulatorbase_num_inonecell][coverage_waypoints_num]=scheduled_selectedjoints_dict[coverage_waypoints_num][0:6]
        ...
        if len(renovation_waypaths_onecell)<=2:
            break
    return renovation_rodclimbing_motiondistance, renovation_manipulatorwaypoint_jointslist
